# =============================================================================
# CSIRO Image2Biomass - Pure CNN (Direct Full Image Input)
# -----------------------------------------------------------------------------
# - 直接整图输入；CNN 主干（ConvNeXt/EfficientNetV2等）作为特征提取器
# - 无 Tile + FiLM 机制，直接全局特征提取
# - 三路 MLP 头 + Softplus 约束正值输出
# - 物理一致加权 MSE 损失
# - SwanLab 全量集成：配置、指标、图像可视化、断点恢复
# - 所有输出/日志/ckpt 都写入本目录（CNN/Pure_CNN）
# =============================================================================
...
class SingleImageDataset(Dataset):
    """
    直接整图输入；返回 (image_tensor, targets_5)
...
class PureCNN(nn.Module):
    """
    Pure CNN with Direct Full Image Input
    - 直接将整图输入 CNN backbone 提取全局特征
    - 无 Tile + FiLM 机制
    - 通过三个回归头预测
    """
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 融合
        feat = self.fusion(feat)  # (B, hidden)
...
def build_model():
    net = PureCNN(dropout=CFG.dropout, hidden_ratio=CFG.hidden_ratio)
    return net
...
def train_one_epoch(model, loader, optimizer, criterion, scaler, epoch, sw_run=None):
    ...
    for step, (x, tgt5) in enumerate(pbar):
...
def run_fold(fold: int, df_wide: pd.DataFrame, sw_project: str, resume_path: Path = None, resume_mode: str = "auto"):
    ...
    train_ds = SingleImageDataset(tr_df, CFG.image_dir, transforms=train_tf, is_train=True)
    valid_ds = SingleImageDataset(va_df, CFG.image_dir, transforms=valid_tf, is_train=True)
    ...
    LOGGER.info(f"[Fold {fold}] Input size: {input_res}x{input_res}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="CSIRO Pure CNN Training")
