# =============================================================================
# CSIRO Image2Biomass - v4 CrossPVT T2T Mamba Inference (10 Models)
# -----------------------------------------------------------------------------
# - 10-model ensemble: 每个 fold 使用 best_wr2.pt + best_loss.pt
# - TTA (原图/水平翻转/垂直翻转)
# - 从 fold_X/checkpoints/ 加载权重
...
class INF_CFG:
    ...
    # Checkpoint 路径（5-fold × 2 checkpoints = 10 models）
    # 支持两种路径格式：
    # 1. fold_0/checkpoints/best_wr2.pt 和 best_loss.pt
    # 2. fold0/checkpoints/best_wr2.pt 和 best_loss.pt
    CKPT_PATTERN_FOLD_X_WR2 = os.path.join(EXPERIMENT_DIR, "fold_{fold}", "checkpoints", "best_wr2.pt")
    CKPT_PATTERN_FOLD_X_LOSS = os.path.join(EXPERIMENT_DIR, "fold_{fold}", "checkpoints", "best_loss.pt")
    CKPT_PATTERN_FOLDX_WR2 = os.path.join(EXPERIMENT_DIR, "fold{fold}", "checkpoints", "best_wr2.pt")
    CKPT_PATTERN_FOLDX_LOSS = os.path.join(EXPERIMENT_DIR, "fold{fold}", "checkpoints", "best_loss.pt")
    N_FOLDS = 5
    
    # 输出
    SUBMISSION_FILE = "submission_10models.csv"
...
def load_model_from_checkpoint(ckpt_path: str, ckpt_name: str = "") -> nn.Module:
    """从 checkpoint 加载模型"""
    print(f"  加载 {ckpt_name}: {os.path.basename(ckpt_path)}")
    ...
    print(f"    ✓ 成功 | backbone={backbone_name} | input_res={input_res}")
...
def find_checkpoint_path(fold: int, ckpt_type: str) -> Optional[str]:
    """查找 checkpoint 路径（支持两种格式）"""
    if ckpt_type == "wr2":
        path1 = INF_CFG.CKPT_PATTERN_FOLD_X_WR2.format(fold=fold)
        path2 = INF_CFG.CKPT_PATTERN_FOLDX_WR2.format(fold=fold)
    elif ckpt_type == "loss":
        path1 = INF_CFG.CKPT_PATTERN_FOLD_X_LOSS.format(fold=fold)
        path2 = INF_CFG.CKPT_PATTERN_FOLDX_LOSS.format(fold=fold)
    else:
        return None
...
def run_inference(test_df: pd.DataFrame, image_dir: str) -> np.ndarray:
    """运行完整推理流程（10-model ensemble + TTA）"""
    print("\n" + "=" * 80)
    print("开始推理 (10-Model Ensemble)")
    print("=" * 80)
    
    # 加载所有 fold 的模型（每个 fold 2 个 checkpoint）
    print("\n加载模型 (5-fold × 2 checkpoints = 10 models)...")
    models = []
    model_info = []  # 记录模型信息
    input_res = None
    ...
    for fold in range(INF_CFG.N_FOLDS):
        ...
        # 加载 best_wr2.pt
        ckpt_wr2_path = find_checkpoint_path(fold, "wr2")
        if ckpt_wr2_path:
            model_wr2 = load_model_from_checkpoint(ckpt_wr2_path, f"best_wr2")
            models.append(model_wr2)
            model_info.append(f"fold{fold}_best_wr2")
            
            # 使用第一个模型确定输入分辨率
            if input_res is None:
                input_res = getattr(model_wr2, "input_res", 518)
                print(f"  输入分辨率: {input_res}")
        else:
            print(f"  ⚠️  best_wr2.pt 不存在")
        
        # 加载 best_loss.pt
        ckpt_loss_path = find_checkpoint_path(fold, "loss")
        if ckpt_loss_path:
            model_loss = load_model_from_checkpoint(ckpt_loss_path, f"best_loss")
            models.append(model_loss)
            model_info.append(f"fold{fold}_best_loss")
        else:
            print(f"  ⚠️  best_loss.pt 不存在")
    ...
    if len(models) == 0:
        ...
        for fold in range(INF_CFG.N_FOLDS):
            print(f"     - {INF_CFG.CKPT_PATTERN_FOLD_X_WR2.format(fold=fold)}")
            print(f"     - {INF_CFG.CKPT_PATTERN_FOLD_X_LOSS.format(fold=fold)}")
...
# =============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description="CSIRO v4 CrossPVT T2T Mamba Inference (10 Models)")
    parser.add_argument(
        "--test-csv",
        type=str,
        default=None,
...
def main():
    ...
    if args.experiment_dir:
        ...
        # 更新 checkpoint 路径模式
        INF_CFG.CKPT_PATTERN_FOLD_X_WR2 = os.path.join(INF_CFG.EXPERIMENT_DIR, "fold_{fold}", "checkpoints", "best_wr2.pt")
        INF_CFG.CKPT_PATTERN_FOLD_X_LOSS = os.path.join(INF_CFG.EXPERIMENT_DIR, "fold_{fold}", "checkpoints", "best_loss.pt")
        INF_CFG.CKPT_PATTERN_FOLDX_WR2 = os.path.join(INF_CFG.EXPERIMENT_DIR, "fold{fold}", "checkpoints", "best_wr2.pt")
        INF_CFG.CKPT_PATTERN_FOLDX_LOSS = os.path.join(INF_CFG.EXPERIMENT_DIR, "fold{fold}", "checkpoints", "best_loss.pt")
    if args.output:
    ...
    print("=" * 80)
    print("CSIRO Image2Biomass - v4 CrossPVT T2T Mamba Inference (10 Models)")
    print("=" * 80)
